        ...
        if (buffer)
        {
            ...
            TraversalGZIPFile((BYTE *)buffer, [=](uint8_t *begin, uint32_t size, uint32_t &new_len) {
                ...
                if (pos)
                {
                    ...
                    const char prouct_title[] = u8R"({aboutBrowserVersion}</div><div class="secondary"><a target="_blank" href="https://github.com/suheandzl/chrome_plus">Chrome++ MOD by SuHeAndZl</a> )" RELEASE_VER_STR u8R"( inside</div>)";
